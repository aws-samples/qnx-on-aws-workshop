...
効率的な QNX アプリケーション開発のために、QNX Toolkit 拡張機能を備えた Kiro IDE を含むモダンな開発ツールを使用します。

シンプルな QNX コックピットアプリケーションを使用して、Kiro IDE でのクロスプラットフォーム QNX 開発を実演します。ワークショップでは、AI 搭載のコード理解、自動ビルドとデプロイメント、Linux と QNX ターゲット間のクロスプラットフォーム互換性について説明します。
